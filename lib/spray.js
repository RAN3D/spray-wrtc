...
Spray.prototype.onStampedTicketRequest = function(id, ticket, peer){
    ...
    socket.on('connect', function(){
        ...
        if (!self.sockets.addSocket(socket, peer)){ socket.destroy(); };
    });
    // #5 closed connection
    socket.on('close', function(){
        console.log('wrtc: a connection has been closed');
    ...
    setTimeout(function(){
        ...
        // (TODO) otherwise create a duplicate at random in the partial view
        if (self.pending.contains({id:id})){
            self.pending.removeSocket({id:id});
            socket.destroy();
        };
...
Spray.prototype.receive = function(message, socket){
    ...
    case 'MStampedTicket':
        ...
        if (!this.forwards.contains(message)){
            // #1 if there is no forwarding route, the stamped ticket is for us
            // to finalize
            ...
            this.forwards.removeSocket(message);
            // (TODO) think about closing the socket here
        };
        break;
    case 'MExchange':
