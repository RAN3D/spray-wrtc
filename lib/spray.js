...
function Spray(options){
    ...
    this.ID = (option && (''+options.id+'')) || GUID();
    ...
    this.neighborhood.inview.on('receive', receive);
    this.neighborhood.outview.on('receive', receive);
};
...
Spray.prototype.connection = function(callbacks, message){
    ...
    var self = this;
    // #1 the contact neighbor
    this.neighborhoods.on('ready', function(idsocket, view){
        if (view!=='inview' && idsocket!==id){return ;};
        ...
        if (outview.length > 0){
            ...
            this.partialView.add({id:message.peer,socketId:id});
        };
    });
};
