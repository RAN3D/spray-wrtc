...
function exchange(){
    ...
    while (!socketOldest && this.partialView.length()>0){
        ...
        (socketOldest && this.send(socketOldest,
    ...
    for (var i = 0; i < sample.length; ++i){
        if (sample[i].id !== oldest.id){
            ...
            this.neighborhoods.connect(oldest.id, sample[i].id);
        } else {
            // #B from oldest to this
...
function onExchange(msg){
    ...
    for (var i = 0; i < sample.length; ++i){
        if (sample[i].id !== msg.id){
            this.neighborhoods.connect(sample[i].id, msg.id);
        } else {
